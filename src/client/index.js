...
const CSS = `
  .tetris-plugin-root {
    ...
    /* Backdrop opacity: lower alpha = more see-through (the DSH UI shows
       through while the game is open). Tweak the 0.55 to taste. */
...
function TetrisOpenButton() {
  return React.createElement(
    'button',
    { className: 'tp-open-btn', type: 'button', onClick: () => setOpen(true), title: '打开俄罗斯方块' },
    '🎮 俄罗斯方块',
  )
}
...
function TetrisGame() {
  ...
  // Inject the plugin stylesheet once; remove it with this component.
  React.useEffect(() => {
  ...
  // ---- game engine (ported from the dynamic-plugin version) ----
  React.useEffect(() => {
    ...
    // ===== Mutable game state =====
    // Note: native browser timers (setTimeout/setInterval/requestAnimationFrame)
    // are fine here — static client plugins are ordinary browser modules.
    const G = {
    ...
    function stopBGM() {
      G.bgmPlaying = false
      if (G.bgmSchedulerId) { clearInterval(G.bgmSchedulerId); G.bgmSchedulerId = null }
    }
    ...
    function onKeyUp(e) {
      ...
      if (id) { clearTimeout(id); delete G.repeatTimers[e.code] }
    }
    ...
    return () => {
      ...
      for (const code in G.repeatTimers) {
        if (G.repeatTimers[code]) clearTimeout(G.repeatTimers[code])
      }
      G.repeatTimers = {}
  ...
  return React.createElement('div', { className: 'tetris-plugin-root', style: open ? null : { display: 'none' } },
    React.createElement('div', { className: 'tp-overlay-inner' },
      React.createElement('button', { className: 'tp-close-btn', type: 'button', onClick: () => setOpen(false), title: '关闭' }, '✕'),
...
export function apply(ctx) {
  // Sidebar open button: always available, launches the game overlay.
  ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register(
    { name: 'sidebar.footer.action', id: 'dsh-tetris-open', order: -1000, label: '俄罗斯方块' },
    () => React.createElement(TetrisOpenButton),
  ))
  // Frame-wide game overlay (click-through layer; the entry opts back into
  // pointer events via its own CSS and renders nothing visible when closed).
  ctx.slots.inject('shell.overlay', () => ctx.slots.register(
    { name: 'shell.overlay', id: 'dsh-tetris-game' },
